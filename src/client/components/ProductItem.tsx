    ...
    return (
        <div data-testid={product.id} className={bem(null, ['card', 'w-100', 'mb-4'])}>
            ...
            <div className="card-body">
                <h5 data-testid="product-title" className={bem('Name', ['card-title'])}>{product.name}</h5>
                <p data-testid="product-price" className={bem('Price', ['card-text'])}>
                    ${product.price}
                </p>
                <Link data-testid="product-link" to={`/catalog/${product.id}`} className={bem('DetailsLink', ['card-link'])}>Details</Link>
